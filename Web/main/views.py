...
def admin_register(request):
    if request.method == "POST":
        ...
        if admin_pass==admin_pass2 :
            user = User.objects.create_user(username=admin_username,email=email,password=admin_pass)
            login(request,user)
            b=Usermanager(uname=admin_username,email=email,level="1002", group='admin_level_1')
            b.save()
